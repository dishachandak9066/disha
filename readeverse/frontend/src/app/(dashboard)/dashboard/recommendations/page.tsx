  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        ...
        // Python backend returns { genres, books }
        // Map to component format with books as personalized recommendations
        if (res && res.books) {
          const mappedBooks = res.books.map((b: any) => ({
            id: String(b.id),
            title: b.title,
          ...
          setData({
            genres: res.genres || [],
            books: mappedBooks,
            personalized: mappedBooks, // Use books as personalized recommendations
            trending: [],
            similar: [],
          });
        } else {
